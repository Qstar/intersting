...
int		command_parser(char *cmd)
{
  ...
  FILE *file = fopen("example.kml","w+");
	if(file == NULL){
		exit(1);
	}
	char *pStr = "<?xml version='1.0' encoding='UTF-8'?>\n"
	...
	char *nStr = "</Point>\n"
				...
				"</Document>\n"
				"</kml>";
	fwrite(pStr,1,strlen(pStr) * sizeof(char),file);
	fwrite("<coordinates>",1,strlen("<coordinates>") * sizeof(char),file);
	fprintf(file,"%f",longitude/100);
	fwrite("E",1,strlen("E"),file);
	fprintf(file,",%f",latitude/100);
	fwrite("N",1,strlen("N"),file);
	fwrite(",0</coordinates>\n",1,strlen(",0</coordinates>\n") * sizeof(char),file);
	fwrite(nStr,1,strlen(nStr) * sizeof(char),file);
	fclose(file);
  	return (0);
}
